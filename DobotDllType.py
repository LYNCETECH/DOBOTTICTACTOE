...
def SetPTPCmdEx(api, ptpMode, toPoint, isQueued=0):
    ret = SetPTPCmd(api, ptpMode, toPoint[0], toPoint[1], toPoint[2], toPoint[3], isQueued)
    while(True):
        if ret[0] <= GetQueuedCmdCurrentIndex(api)[0]:
            break;
        dSleep(5)

def SetARCCmdEx(api, cirPoint, toPoint,  isQueued=0):
